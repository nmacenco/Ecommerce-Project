...
import { CardsGroup } from "./CardsStyles";
...
const Cards = (): JSX.Element => {
  ...
  const finalProduct = currentPage * 12;
  const firstProduct = finalProduct - 12;
  const productList = data.slice(firstProduct, finalProduct);

  return (
    <div className="w-100">
      <Filter />
      <div className="mx-auto mt-1 row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-xl-4 g-4">
        {productList.map((e) => {
          return (
      ...
      <Pagination length={data.length} page={page} />
    </div>
  );
};
