...
const createProduct = async (req, res, next) => {
  try {
    ...
    if (
      ...
      res
        .status(400)
        .json({ errorMsg: "Information isn't enough to create the product" });
    } else {
...
const updateProduct = async (req, res, next) => {
  try {
    ...
    productUpdated
      ? res.status(201).json({
          successMsg: "The Product has been Updated",
          data: productUpdated,
        })
      : res.status(404).json({ errorMsg: "The Product can not be created" });
  } catch (error) {
...
const getSingleProduct = async (req, res, next) => {
  try {
    ...
    productSingle
      ? res
          .status(201)
          .json({ successMsg: "Data found in Database", data: productSingle })
...
const getProducts = async (req, res, next) => {
  try {
    ...
    dataProduct
      ? res
          .status(201)
          .json({ successMsg: "Data found in Database", data: dataProduct })
      : res.status(404).send({ errorMsg: "The product doesn't found" });
  } catch (error) {
    res.status(500).send({ errorMsg: error.message });
  }
